...
#rem importing injection code --FFG 
#all my edits are marked with #rem
from .kn5_sanitizer import del_name_dot_index
...
#rem TODO: adding del_name_dot_index,changable for performance
...
def fbx_data_from_scene(scene, depsgraph, settings):
    ...
    for ob_obj in objects:
        ...
        for ma_s in ob_obj.material_slots:
            ...
            #
            # print('get_blenderID_key(ma) is: ' + get_blenderID_key(ma))
            ma_clean = del_name_dot_index(get_blenderID_key(ma))
            # print('ma_clean is: ' + ma_clean)
            #rem creating ma_data_temp to do away with this tupple stuff
            ma_data_temp = data_materials.setdefault(ma, (ma_clean, []))
            #ma_data = data_materials.setdefault(ma, (ma_clean, []))
            
            #ma type is material            
            print('----')
            #rem sanitize material name done
            print('ma_data_temp[0] before: ' + ma_data_temp[0])
            ma_data = [del_name_dot_index(ma_data_temp[0]), ma_data_temp[1]]     #rem gone, begone!
            print('ma_data[0] after: ' + ma_data[0])
    ...
    # Textures
    for (ma, sock_name), (tex_key, fbx_prop) in data_textures.items():         #rem fixing duplicate materials all revert to using name of mat.001 instead of mat
...
def fbx_objects_elements(root, scene_data):
    ...
    for ob_obj in scene_data.objects:
        if ob_obj.is_dupli:
            continue
        #rem using del_name_dot_index() to sanitize indexes in ob_obj.name here.
        
        print("----------------")
        print("starting with: " + ob_obj.name)
        ob_obj.name = del_name_dot_index(ob_obj.name)
        print("ending with: " + ob_obj.name)
        print("----------------")
